...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    model.paintColor = [_lineColor toColorOneString];
            ...
            switch (type) {
                ...
                case setTypeBack:
                {
                    ...
                    if (self.paths.count == 0) {
                        [SVProgressHUD showErrorWithStatus:@"Last, not retreat"];
                        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                            [SVProgressHUD dismiss];
                        });
                        NSLog(@"已经最后一张了，不能撤退了");
                ...
                case setTyperegeneration:
                {
                    ...
                    if (self.tempPath.count == 0) {
                        ...
                        [SVProgressHUD showErrorWithStatus:@"Have the latest one, not rolled back"];
                        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                            [SVProgressHUD dismiss];
                        });
                        NSLog(@"已经最新一张了，不能回滚了");

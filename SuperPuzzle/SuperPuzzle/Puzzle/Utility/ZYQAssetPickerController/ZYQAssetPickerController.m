...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row==ceil(self.assets.count*1.0/columns)) {
        ...
        if (_numberOfVideos == 0)
            ...
            title = [NSString stringWithFormat:NSLocalizedString(@"%ld viedos", nil), (long)_numberOfVideos];
        else
...
- (void)setTitleWithSelectedIndexPaths:(NSArray *)indexPaths
{
    ...
    if (photosSelected && videoSelected)
        ...
        format = (indexPaths.count > 1) ? NSLocalizedString(@"choose %ld videos", nil) : NSLocalizedString(@"choose %ld videos ", nil);
